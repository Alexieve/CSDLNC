...
BEGIN
	...
	BEGIN 
		...
		END
		BEGIN TRY
			INSERT INTO LICHHEN VALUES (@MANS, @MAHSBN, @HOTENBN, @SDTBN, @HOTENNS, @NGAYHEN, @GIOHEN, 0, NULL)
		END TRY
		BEGIN CATCH
...
CREATE OR ALTER PROC SP_GET_INFOR_TAIKHAM
	@MAKHDIEUTRI INT
AS
BEGIN
	DECLARE @MAHSBN INT
	SELECT @MAHSBN = MAHSBN
	FROM KHDIEUTRI 
	WHERE MADIEUTRI = @MAKHDIEUTRI

	DECLARE @SDTBN CHAR(10)
	SELECT @SDTBN = SDTBN
	FROM HOSOBN
	WHERE MAHSBN = @MAHSBN

	SELECT MAKHDIEUTRI AS MATAIKHAM, @MAHSBN AS MAHSBN, HOTENBN, @SDTBN AS SDTBN, KHAMCHINH AS MANS, TENKHAMCHINH AS HOTENNS
	FROM KHDIEUTRI
