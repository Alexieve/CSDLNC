...
BEGIN
	...
	LEFT JOIN
		LICHHEN LH ON NS.MANS = LH.MANS
					AND LH.NGAYHEN = @NGAYHEN
					AND LH.GIOHEN = @GIOHEN
	LEFT JOIN 
		LICHNGHI LN ON NS.MANS = LN.MANS 
					AND LN.NGAYNGHI = @NGAYHEN
	WHERE NS.MACN = @MACN
		AND LH.MANS IS NULL
		AND LN.MANS IS NULL
		AND LLV.NGAYLAM = @DAYWEEK
		AND @GIOHEN BETWEEN LLV.GIOBATDAU AND LLV.GIOKETTHUC 
		AND (CONVERT(NVARCHAR(MAX), NS.MANS) LIKE @KEYWORD
			OR NS.HOTEN LIKE @KEYWORD2)
...
BEGIN
	...
	-- Số cuộc hẹn sắp tới
	DECLARE @CARD1 INT
	SELECT @CARD1 = COUNT(MALH)
	FROM LICHHEN LH JOIN #HSBN HSBN ON LH.MAHSBN = HSBN.MAHSBN
	WHERE LH.XACNHAN = 1
	...
	DECLARE @CARD3 INT
	SELECT @CARD3 = COUNT(MAKHDIEUTRI)
	FROM KHDIEUTRI KH JOIN #HSBN HSBN ON KH.MAHSBN = HSBN.MAHSBN
	WHERE KH.TRANGTHAI = 1

	--Số kế hoạch điều trị đã hoàn thành
	DECLARE @CARD4 INT
	SELECT @CARD4 = COUNT(MAKHDIEUTRI)
	FROM KHDIEUTRI KH JOIN #HSBN HSBN ON KH.MAHSBN = HSBN.MAHSBN
	WHERE KH.TRANGTHAI = 2
	
	--Tổng tiền điều trị, Số tiền đã thanh toán
	DECLARE @CARD5 INT
	DECLARE @CARD6 INT
	SELECT @CARD5 = SUM(HSBN.TONGTIENDIEUTRI), @CARD6 = SUM(HSBN.DATHANHTOAN)
	FROM HOSOBN HSBN JOIN #HSBN HSTMP ON HSBN.MAHSBN = HSTMP.MAHSBN

	--Số nha sĩ trên hệ thống
	DECLARE @CARD7 INT
	SELECT @CARD7 = COUNT(MANS)
	FROM NHASI

	--Số hồ sơ bệnh nhân của bản thân
	DECLARE @CARD8 INT
	SELECT @CARD8 = COUNT(MAHSBN)
	FROM #HSBN
