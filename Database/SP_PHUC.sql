...
AS
BEGIN
	DECLARE @KEYWORD1 NVARCHAR(MAX) = @KEYWORD + '%'
	DECLARE @KEYWORD2 NVARCHAR(MAX) = '%' + @KEYWORD1
	...
	BEGIN
		...
		FROM HOSOBN
		WHERE
			CONVERT(NVARCHAR(MAX), MAHSBN) LIKE @KEYWORD1 OR
...
BEGIN
	...
	FROM NHASI
	WHERE
		CONVERT(NVARCHAR(MAX), MANS) LIKE @KEYWORD1 OR
		HOTEN LIKE @KEYWORD2
	RETURN
END
GO