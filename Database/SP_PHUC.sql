...
BEGIN
	BEGIN TRY
		IF (@SOLUONG < (SELECT SL FROM THUOC WHERE @MATHUOC = MATHUOC))
			BEGIN
				RAISERROR('Số lượng để kê đơn vượt qua số còn lại trong kho!', 16, 1)
				RETURN
			END
		DECLARE @TIEN INT;
		SET @TIEN = (SELECT DONGIA FROM THUOC WHERE MATHUOC = @MATHUOC)
		INSERT INTO CHITIET_DONTHUOC VALUES(@MAKHDIEUTRI,@MATHUOC,@SOLUONG,@TIEN)
		UPDATE THUOC
		SET SL = SL - @SOLUONG
