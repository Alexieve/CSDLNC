        ...
        * */
        ...
        var eventBinding = function(){
            ...
            thisSheet.delegate(".TimeSheet-cell","mouseup.umsSheetEvent",function(ev){
                ...
                for (var row = correctedCells.topLeft[0]; row <= correctedCells.bottomRight[0]; ++row) {
                    publicAPI.setContinuousCellsInRow(row, sheetModel.getCellState([row, correctedCells.topLeft[1]]));
                }
            });
        ...
        var publicAPI = {
            ...
            * */
            ...
            setCellState: function(cellIndex, state){
                sheetModel.set(state, {
                    startCell: cellIndex,
                    endCell: cellIndex
                });
                repaintSheet();
                if(sheetOption.end){
                    sheetOption.end(null, { topLeft: cellIndex, bottomRight: cellIndex });
                }
            },

            setContinuousCellsInRow: function(row){
                var rowStates = publicAPI.getRowStates(row);
        
                var firstSelectedIndex = rowStates.indexOf(1);
                var lastSelectedIndex = rowStates.lastIndexOf(1);
        
                if (firstSelectedIndex !== -1 && lastSelectedIndex !== -1 && firstSelectedIndex !== lastSelectedIndex) {
                    // At least two selected cells are found in the row
        
                    var startIndex = Math.min(firstSelectedIndex, lastSelectedIndex);
                    var endIndex = Math.max(firstSelectedIndex, lastSelectedIndex);
        
                    for (var col = startIndex + 1; col < endIndex; ++col) {
                        publicAPI.setCellState([row, col], 1);
                    }
                }
            },
